...
TrialResults(50).filter(3).test_vpts = [];


%% load the data

%load data
load('KS_filtered_data_20.mat')
...
for nnn = 1:50
    ...
    for every_day_is_a_blessing = [1,2,3]
        ...
        %go through each filter (na, fourier, wdenoise, poly)
        if every_day_is_a_blessing == 1

            %training
            X_train = u_train_output(1:end-1,:,nnn);
            Y_train = u_train_output(2:end,:,nnn);
        ...
        elseif every_day_is_a_blessing == 2
            ...
            Y_train = u_train_filter_wdenoise_output(2:end,:,nnn);
        ...
        % figure(every_day_is_a_blessing*4+1)
        ...
        % title('Mean')
        % 
        % figure(every_day_is_a_blessing*4+2)
...
%save everything
save('KS_DM_strong_20.mat', 'TrialResults', '-v7.3');
