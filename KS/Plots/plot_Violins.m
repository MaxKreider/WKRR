...
mean_vpts_weakDM_noisy = reshape([val_weakDM_train_noisy_results.mean_vpt], 1, 50);
mean_vpts_weakDM_filter = reshape([val_weakDM_train_filter_results.mean_vpt], 1, 50);
...
VPT_Comparison_Table_T = array2table(stats_matrix, ...
    ...
    'RowNames', {'Strong (n/a)', 'Strong (Wavelet)', 'Strong (Poly)', 'Weak (n/a)', 'Weak (Poly)', 'Weak DM (n/a)', 'Weak DM (Poly)'});
...
nexttile;
hold on;
vpt_data_to_plot = [vpt_list_strong_na', vpt_list_strong_wavelet', vpt_list_strong_poly', vpt_list_weak_noisy', vpt_list_weak_filter', vpt_list_weakDM_noisy', vpt_list_weakDM_filter'];
...
v(3).FaceAlpha = 0.5;
v(4).FaceColor = [0.75, 0.60, 0.85];    
v(4).FaceAlpha = 0.5;
v(5).FaceColor = [0.45, 0.20, 0.60]; 
v(5).FaceAlpha = 0.5;
v(6).FaceColor = [1, 0.45, 0.45]; 
v(6).FaceAlpha = 0.5;
v(7).FaceColor = [1, 0, 0]; 
v(7).FaceAlpha = 0.5;
...
line([6-line_w, 6+line_w], [med_weakDM_na, med_weakDM_na], 'Color', 'k', 'LineWidth', 3);
line([7-line_w, 7+line_w], [med_weakDM_poly, med_weakDM_poly], 'Color', 'k', 'LineWidth', 3);

%formatting
xticklabels({'Strong (n/a)', 'Strong (Wavelet)','Strong (Poly)','Weak (n/a)','Weak (Poly)', 'Weak DM (n/a)', 'Weak DM (Poly)'});
ylabel('VPT');
grid on;
set(gca, 'FontSize', 14);
box on
