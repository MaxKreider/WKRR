...
%load the four cases
S = load('KS_weak_val_A_01.mat');
results_50 = [S.TrialResults.filter];

S = load('KS_weak_val_B_01.mat');
results_100 = [S.TrialResults.filter];

S = load('KS_weak_val_C_01.mat');
results_200 = [S.TrialResults.filter];

S = load('KS_weak_val_D_01.mat');
results_400 = [S.TrialResults.filter];

S = load('KS_weak_val_E_01.mat');
...
%extract the VPTs
mean_vpts_50 = reshape([results_50.mean_vpt], 1, 50);
mean_vpts_100 = reshape([results_100.mean_vpt], 1, 50);
mean_vpts_200 = reshape([results_200.mean_vpt], 1, 50);
mean_vpts_400 = reshape([results_400.mean_vpt], 1, 50);
mean_vpts_500 = reshape([results_500.mean_vpt], 1, 50);


%% visualize

%organize data into arrays
L_values = [50, 100, 200, 400, 500] * 0.1 * 0.043
