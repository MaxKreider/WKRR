...
TrialResults(100).filter(2).test_vpts = [];
...
for nnn = 1:100
    ...
    %training
    X_train = u_train_true_output(1:end-1,:,nnn);
    Y_train = diff(u_train_true_output(:,:,nnn));

    %validation
    u_validation = u_val_true_output(:,:,nnn);
...
%save everything
save('L63_strong_00.mat', 'TrialResults', '-v7.3');
