...
if nlevel == 0.01
    p = 12; L = 60*dt; overlap = 0.1;
elseif nlevel == 0.05
    p = 5;  L = 40*dt; overlap = 0.2;
elseif nlevel == 0.1
    p = 5;  L = 50*dt; overlap = 0.2;
elseif nlevel == 0.2
    p = 5;  L = 50*dt; overlap = 0.2;
end
...
%training
X_train = u_train_filter_poly_output(:,:,nnn);
Y_train = diff(X_train);
...
u_test_traj = u_test_true(idx:idx+40000,:);
...
for d = 1:3
    ...
    plot(t_test_traj(1:1000)*Lambda, k_sol(1:1000, d), '--', 'Color', [0.80, 0.40, 1.00], 'LineWidth', 2.5);
    xline(my_VPT,'g', 'LineWidth', 3);
end
...
plot3(k_sol(end-ii:end,1), k_sol(end-ii:end,2), k_sol(end-ii:end,3), '--', 'Color', [0.80, 0.40, 1.00], 'LineWidth', 3);
