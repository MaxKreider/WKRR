...
for nnn = 1:100
            ...
            for jjj = 1:length(lambda_mesh)
                ...
                for kkk = 1:length(val_start_indices)
                    ...
                    for i=1:length(k_sol) - 1
                        ...
                        %iterate the model
                        k_sol(i+1,:) = x_current +  kernel_func(x_current,X_train,N,q,q_hat,epsilon_mesh(iii),alpha,X_sq_const.Value);
    ...
    parfor ppp = 1:length(test_indices)
        ...
        for i=1:length(k_sol)-1
            ...
            %iterate the model
            k_sol(i+1,:) = x_current + kernel_func(x_current,X_train,N,q,q_hat,epsilon_final,alpha,X_sq_const.Value);
...
%save everything
save('L63_DM_weak_00.mat', 'TrialResults', '-v7.3');
