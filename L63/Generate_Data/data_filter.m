...
for n = 1:4
    ...
    %training
    data{n}.u_train = zeros(N_train_new, 3, num_iterations);
    data{n}.u_train_filter_wdenoise = zeros(N_train_new, 3, num_iterations);
    data{n}.u_train_filter_poly = zeros(N_train_new, 3, num_iterations);
    data{n}.u_train_true = zeros(N_train_new, 3, num_iterations);

    %validation
    data{n}.u_val = zeros(N_val_new, 3, num_iterations);
    data{n}.u_val_filter_wdenoise = zeros(N_val_new, 3, num_iterations);
    data{n}.u_val_filter_poly = zeros(N_val_new, 3, num_iterations);
    data{n}.u_val_true = zeros(N_val_new, 3, num_iterations);

    %testing
    data{n}.u_test = zeros(N_test_new, 3, num_iterations);
    data{n}.u_test_true = zeros(N_test_new, 3, num_iterations);

    %error metrics
    data{n}.error_angle = zeros(3,num_iterations);
    data{n}.error_rmse = zeros(3,num_iterations);
    data{n}.error_LSD = zeros(3,num_iterations);
end

%loop over all noise levels
for nnn = 1:4
    
    %load the data and appropriate polynomial parameters
    if nnn == 1

        %data
        load('L63_data_01.mat')
    ...
    for qqq = 1:100
        ...
        u_train_current = u_train(:,:,qqq);
        u_train_true_current = u_train_true(:,:,qqq);
        ...
        LSD_errors = [LSD_na/3; LSD_wdenoise/3; LSD_poly/3];
    ...
    filename_data = sprintf('L63_filtered_data_%s.mat', file_suffixes{nnn});
    filename_error = sprintf('L63_filtered_data_error_%s.mat', file_suffixes{nnn});
    ...
    %and for the error
    angle_output = data{nnn}.error_angle;
    rmse_output = data{nnn}.error_rmse;
    LSD_output = data{nnn}.error_LSD;
    ...
    save(filename_error,'angle_output','rmse_output','LSD_output')
end
